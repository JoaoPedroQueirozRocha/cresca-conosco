	...
	methods: {
		async handleDelete() {
			const result = await this.$confirm({
				...
				description:
					"Ao apagar sua conta, todos os dados cadastrados serão perdidos para sempre.",
			});
			if (!result) return;
			try {
		...
		isValidPass() {
			return (
				this.passObject.actualPass &&
				this.passObject.newPass &&
				this.passObject.confirmNewPass
			);
		},
	...
	<div class="flex flex-col gap-6 w-full">
		...
		<Card class="w-full flex flex-col gap-4">
			...
			<div class="flex flex-col gap-4">
				...
				<Input
					...
					type="email"
				/>
				<Button class="w-fit" @click="changingPass = true"
		...
		<Dialog v-model="changingPass" @update:model-value="resetPassForm">
			<div class="flex flex-col xs:gap-6 gap-4 p-2">
				<h1
					class="xs:text-4xl text-2xl font-bold whitespace-normal break-words"
				>
					Alterar Senha
				</h1>
				<div class="w-full flex flex-col gap-2">
					<Input
						v-model="passObject.actualPass"
						label="Senha Atual"
						type="password"
						class="input-pass"
					/>
					<Input
						v-model="passObject.newPass"
						label="Nova Senha"
						type="password"
						class="input-pass"
					/>
				...
				<div class="flex gap-2 justify-center xs:flex-row flex-col">
					<Button only-border :size="size" @click="resetPassForm"
						>Cancelar</Button
					>
