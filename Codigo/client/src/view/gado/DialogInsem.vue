<template>
  ...
  >
    <div
      class="dialog-div"
      :class="{ bigger: dateOpened.data_insem || dateOpened.prev_parto }"
    >
      <div>
      ...
      <div class="flex flex-row gap-4 justify-end">
        <Button @click="cancelar" only-border :disabled="loading"
          >Cancelar</Button
        >
    ...
    animalData() {
      if (this.animalData.status) {
        this.gestacaoData = {
          ...
          data_insem: this.animalData.data_insem
            ? new Date(this.animalData.data_insem)
            : null,
          prev_parto: this.animalData.prev_parto
            ? new Date(this.animalData.prev_parto)
            : null,
        };
      } else {
    ...
    async salvarGestacao() {
      if (!this.validateData(this.gestacaoData)) {
        ...
        try {
          ...
          if (this.isEdit)
            await gestacaoController.editarGestacao(
              this.gestacaoData.animal_id,
              this.gestacaoData
            );
